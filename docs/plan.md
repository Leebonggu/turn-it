...
- [x] **사이클 히스토리**: 과거 사이클 목록 + 아이디어 재조회.
- [ ] **질문 풀 확장**: 10개 → 30개 (카테고리별 균형 배분).
- [ ] **로딩 상태 세분화**: 아이디어 생성 중 진행률/단계 표시.

### 안정성 & 에러 처리 (높은 우선순위)

- [x] **AI 재시도 개선**: 지수 백오프 + 네트워크 에러 구분 + 429 rate limit 핸들링.
- [x] **Firestore 에러 처리**: 모든 Firestore 호출에 try-catch + 사용자 피드백.
- [x] **입력 유효성 검사**: 이메일 형식, 비밀번호 강도, 불만 내용 빈칸/공백 체크, 사이클 이름 등.
- [x] **중복 제출 방지**: 아이디어 생성, 불만 저장 버튼 연타 방지 (디바운스/로딩 중 비활성화).
- [ ] **Gemini 토큰 제한 체크**: API 호출 전 입력 크기 검증, 초과 시 경고.

### 성능 (높은 우선순위)

- [x] **N+1 쿼리 해결**: cycle-history에서 사이클당 complaints/ideas 개별 조회 → 배치/집계로 변경.
- [ ] **불만 목록 페이지네이션**: complaints 탭에서 전체 로드 → 무한스크롤.
- [ ] **아이디어 목록 페이지네이션**: ideas 탭에서 전체 로드 → 무한스크롤.
- [ ] **컴포넌트 메모이제이션**: ComplaintItem, IdeaCard에 React.memo 적용.
- [ ] **FlatList 최적화**: renderItem에 useCallback, getItemLayout 적용.

### UX 개선 (중간 우선순위)

- [ ] **불만 검색/필터**: 태그별, 날짜별, 키워드 검색.
- [ ] **아이디어 공유/내보내기**: PDF 또는 공유 링크로 아이디어 내보내기.
- [ ] **아이디어 상태 확장**: 'interested'/'pending'/'discarded' → 'building'/'implemented' 추가.
- [ ] **아이디어 우선순위**: 1-5 우선순위 + 정렬.
- [ ] **스와이프 삭제 + 되돌리기**: 불만/아이디어 스와이프 삭제 + undo 토스트.
- [ ] **아이디어 리파인**: AI에게 아이디어 보완/확장 요청.
- [ ] **사이클 상세 스켈레톤**: cycle-detail 화면에 스켈레톤 로딩.
- [ ] **온보딩 개선**: 3→7 시스템 설명, 예시 데이터, 컨텍스트 툴팁.
- [ ] **알림 시간 설정 시점**: 첫 로그인이 아니라 첫 불만 기록 후로 이연.

### 데이터 모델 개선

- [ ] **updatedAt 필드**: 모든 문서에 updatedAt 타임스탬프 추가.
- [ ] **소프트 삭제**: 하드 삭제 → deletedAt 플래그로 변경.
- [ ] **아이디어 태그**: 아이디어에도 카테고리 태그 추가.

### 모니터링 & 테스트

- [ ] **에러 로깅**: Sentry 또는 Firebase Crashlytics 연동.
- [ ] **분석 이벤트**: 주요 사용자 행동 추적 (기록 생성, 아이디어 조회 등).
- [ ] **컴포넌트 테스트**: @testing-library/react-native로 주요 화면 테스트.
- [ ] **Firebase 모킹**: jest.mock으로 Firestore 테스트 분리.
- [ ] **테스트 커버리지**: CI/CD에 커버리지 리포트 설정.

### 보안

- [ ] **프롬프트 인젝션 방지**: 불만 내용에서 특수 문자/명령어 필터링.
- [ ] **Firestore 규칙 강화**: list 쿼리에 userId 필터 강제 (request.query 기반).
- [ ] **클라이언트 레이트 리밋**: 하루 최대 불만 개수 제한 (예: 50개).
